...
module.exports = ({ markdownAST }) => {
  visit(markdownAST, "blockquote", node => {
    ...
    if (headingEndBlock == -1 && headingEndPos == -1) {
      headingEndBlock = node.children[0].children.length - 1;
      headingEndPos = node.children[0].children[headingEndBlock].value.length;
    }

    // 从中提取出标题部分
    const heading = node.children[0].children.slice(0, headingEndBlock);
